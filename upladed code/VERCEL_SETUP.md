...
1. Open `https://YOUR-DOMAIN/api/state?health=1` and confirm it returns `{ "ok": true, ... }`.
2. Check Vercel logs for `/api/state`.
3. Recheck `MONGODB_URI`, `MONGODB_DB`, `APP_ACCESS_CODE`.
4. Confirm Atlas user has read/write access and IP `0.0.0.0/0` exists.
5. Redeploy once.
