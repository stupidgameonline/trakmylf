...
const getHealthQueryValue = (req) => {
  const fromQuery = req?.query?.health;
  if (Array.isArray(fromQuery)) return fromQuery[0];
  if (fromQuery !== undefined && fromQuery !== null) return fromQuery;

  try {
    const parsed = new URL(req.url || '', 'http://localhost');
    return parsed.searchParams.get('health');
  } catch {
    return null;
  }
};
...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'GET' && String(getHealthQueryValue(req) || '') === '1') {
